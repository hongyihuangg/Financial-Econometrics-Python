...
1. Train (Standardized + Logarithmic Alpha Grid per Lecture 2)
2. Predict (Saves to pred_oos_ridge.csv)
3. Report (Generates 4-Panel Report + Forecast Plot)
"""
import pandas as pd
import numpy as np
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # We only ffill variables that actually close on weekends (TradFi)
    tradfi_cols = [
    ...
    # General ffill for any minor crypto gaps
    df = df.ffill()
    ...
    # --- RSI CALCULATION ---
    # We include RSI for Ridge too, as it's a valid signal
    ...
    # Drop Raw Levels (Ridge hates non-stationary data)
...
def get_feature_importance(model, feature_names):
    ...
    if hasattr(regressor, 'coef_'):
        ...
        # For Ridge, magnitude matters (positive or negative)
...
def plot_forecast_vs_actual():
    ...
    plt.plot(df['time'], df['signal'], label='Ridge Forecast', color='#ff7f0e', marker='.', markersize=4, linestyle='-', linewidth=2)
    
    plt.title("Ridge Returns: Forecast vs Actual", fontsize=14)
...
def run_training():
    ...
    # 1. Pipeline: MUST Include Scaling for Ridge (Lecture 2)
    pipeline = Pipeline([
        ('scaler', StandardScaler()),
        ('model', Ridge())
    ])
    
    # 2. Hyperparameters: Logarithmic scale per Lecture 2 
    param_grid = {
