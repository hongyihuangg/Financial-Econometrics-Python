...
3. Trains Random Forest (Grid Search for Robustness)
...
def prepare_data():
    ...
    # Even though README says it's Next Day return, the 42% R2 proves it is currently Current Day return.
    # We shift(-1) to force the model to predict TOMORROW's return using TODAY's data.
...
def plot_forecast_vs_actual():
    # Only try to plot if we have predictions in memory
    pass 
...
def run_training():
    ...
    # Pipeline: No StandardScaler needed for RF
    pipeline = Pipeline([
    ...
    is_file = PRED_RESULT_DIR / "rf_pred_is.csv"
    oos_file = PRED_RESULT_DIR / "rf_pred_oos.csv"
